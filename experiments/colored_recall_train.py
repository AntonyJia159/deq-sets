...
MODE = os.environ.get("CR_MODE", "latest")          # 'latest' (recency) or 'earliest' (write-once, long-range)
CKPT_NAME = f"colored_recall_{MODE}.pt"
...
def cr_recall(model, n_items, gen, reps=4, fill=None):
    ...
    for _ in range(reps):
        ...
        toks, qmask, targ, _ = gen_colored_recall(128, n_items, gen, fill=fl, mode=MODE)
        with torch.no_grad():
...
def cr_train(model, n_items, steps, gen, bs=64, lr=3e-3):
    ...
    for st in range(steps):
        ...
        toks, qmask, targ, _ = gen_colored_recall(bs, n_items, gen, fill=Fill, mode=MODE)
...
def main():
    ...
    print(f"device={sw.DEV}  colored register recall (C={C} colors, V={V} values, fixed transform), "
          f"MODE={MODE}, relative-PE causal.\n  Window curriculum then LENGTH curriculum n_items=4->8->12->16 "
          f"({'first-occurrence write-once, LONG-range' if MODE=='earliest' else 'latest-write recency'} "
    ...
    for n in [4, 8, 12, 16]:
        ...
        r, smin, rs = m.spectrum(gen_colored_recall(1, n, torch.Generator().manual_seed(7), fill=8, mode=MODE)[0])
        m.train()
    ...
    torch.save({"state_dict": m.state_dict(), "lengen": lg, "H": sw.H, "W": sw.W, "C": C, "V": V,
                ...
                "no_posw": True, "mlp": False, "task": "colored_recall", "mode": MODE},
